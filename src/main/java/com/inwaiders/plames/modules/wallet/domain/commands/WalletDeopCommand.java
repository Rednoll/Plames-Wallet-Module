...
public class WalletDeopCommand extends MessengerCommand {

	public WalletDeopCommand() {
	
		this.addAliases("deop");
	}
	...
	public void run(UserProfile profile, String... args) throws CommandException {
		...
		if(args.length != 3) {
			...
			throw new CommandException("$wallet.command.deop.invalid_args");
		}
		...
		DescribedFunctionResult result = account.deopUser(suspectAdmin, user);
